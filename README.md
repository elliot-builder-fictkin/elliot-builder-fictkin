<img width="1640" height="200" alt="tumblr_cc22acf8cb61c20f69c7d47ee9256b9b_163b34aa_2048" src="https://github.com/user-attachments/assets/8f8758f0-dee9-4677-92d6-e8c63af00f05" />


<p align="center"><img width="400" height="80" alt="tumblr_1878633c7535a6525cbc8297a529beeb_2f02c8fa_1280" src="https://github.com/user-attachments/assets/3d04a2e3-a404-4975-bd68-b3730528d19d" />
<div></div>

<p align="center"><a href="https://hits.sh/github.com/elliot-builder-fictkin/"><img alt="Hits" src="https://hits.sh/github.com/elliot-builder-fictkin.svg?style=for-the-badge&label=%E0%BC%98%E2%8B%86%20CUSTOMERS%20SERVED%20!&extraCount=200&color=E4B883&labelColor=972A5D"/></a>

hi so dont freak out but i MIGHT like actually die so that’s why if you notice im inactive. to my oomfies and Regis ty for being the reason i stayed longer you guys are awesome, sorry i made my git ugly EITH this message 

<p align="center">$\color{#972A5D}major \space wip \space rn \space gulps,, \space i \space also \space do \space not \space know \space how \space to \space code,,, \space im \space trying \space to \space learn \space from \space some \space guy \space from \space youtube$

<p align="center">$\color{#972A5D}Hello! \space My \space name \space is \space indeed \space Elliot, \space please \space refer \space to \space me \space as \space Elliot!$

<p align="center">$\color{#972A5D}I \space am \space a \space hvykin \space of \space Elliot \space from \space WAAPP/ \space Forsaken \space interchangebly \space as \space a \space coping \space method!$

<p align="center">$\color{#A43557}I \space am \space a \space 20 \space years \space old \space transmac \space demiboy \space who \space studies \space law \space !!$ 

<p align="center">$\color{#A43557}I \space am \space unfortunately \space in \space the \space US \space and \space in \space EST \space !!$
  
<div align="center">
<details>
  <summary>
    $\color{#B14251}DNI/IWC$
  </summary>
</p>

 <p align="center">$\color{#B14251}DNI \space !!$

 <p align="center">$\color{#BE5551}- \space T.R.A.S.H$
 <p align="center">$\color{#BE5551}- \space "Copytown"$
 <p align="center">$\color{#BE5551}- \space Spawnists $
 <p align="center">$\color{#BE5551}- \space MAGA \space or \space Trump \space Supporters$
 <p align="center">$\color{#BE5551}- \space Problematic \space people$
 <p align="center">$\color{#BE5551}- \space MCYT \space Boundary \space Breakers \space and \space anyone \space who \space supports \space or \space enables \space it$  
 <p align="center">$\color{#BE5551}- \space Anyone \space below \space the \space age \space of \space 15, \space excluding \space those \space in \space my \space regiments$  
 <p align="center">$\color{#CB7660}- \space I \space will \space not \space hesitate \space to \space block \space you \space if \space you \space meet \space any \space of \space these \space criteria.$

  <p align="center">$\color{#CB7660}IWC \space !!$ 
  <p align="center">$\color{#CB7660}- \space Elliot \space Kins \space and \space Yumes$
  <p align="center">$\color{#CB7660}- \space Elliot \space Shippers \space (unless \space friends)$
 </details>

 <div align="center">
<details>
  <summary>
    $\color{#D79771}BYI$
  </summary>
</p>

<p align="center">$\color{#D79771}- \space i \space am \space a \space dog \space kin! \space however, \space it \space is \space not \space meant \space to \space be \space a \space petplay \space kind \space of \space thing! \space i \space just \space find \space comfort \space in \space dogs \space and \space relate \space to \space them!$
<p align="center">$\color{#D79771}- \space i \space don't \space mind \space jokes \space but \space please \space keep \space it \space SFW \space unless \space you're \space a \space friend \space and \space above \space the \space age \space of \space 18!$
<p align="center">$\color{#D79771}- \space i \space am \space typically \space off-tab \space or \space afk \space so \space always \space w2i \space or \space i \space likely \space won't \space get \space your \space message!$
<p align="center">$\color{#D79771}- \space can \space typically \space be \space sighted \space in \space Elliot \space skins! \space idm \space if \space you \space come \space up \space to \space me \space to \space talk \space or \space sit$
<p align="center">$\color{#D79771}with \space me \space but \space please \space don't \space cover \space me \space or \space my \space friends \space or \space interrupt \space me \space when \space i'm \space trying \space to \space have \space a \space conversation \space with \space them!$
<p align="center">$\color{#E4B883}- \space always \space open \space to \space C*H \space and \space interactions \space unless \space my \space name \space says \space otherwise!$
 <p align="center">$\color{#E4B883}- \space last \space note!! \space please \space do \space not \space copy \space my \space skins! \space they \space are \space a \space kind \space of \space coping \space link \space for \space me \space and \space i \space do \space not \space want \space them \space stolen!$
<p align="center">$\color{#E4B883}- \space however, \space if \space you'd \space like \space just \space light \space inspo \space please \space ask \space first!$
 </details>

<p align="center">$\color{#E4B883}psssst \space ! \space the \space badges \space direct \space to \space my \space straw, \space ata, \space and \space prnscc!! \space ⤵︎$
<break></break>


<div>
  <a href="https://localelliotkins.straw.page/">
  <img src="https://github.com/user-attachments/assets/576bc861-6aba-4aa8-bd56-fab1a6f3e1c1" /> 
<a href="https://elliotbuilders.atabook.org/">  
  <img src="https://github.com/user-attachments/assets/66434ed3-2817-445c-b7ff-6344d1af90fc">
<a href="https://pronouns.cc/@ellioto">  
  <img src="https://github.com/user-attachments/assets/0da25e8f-9287-4dd4-85f5-aaf42f98f67c">
</a>
</p>


<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.demolab.com?font=The+Girl+Next+Door&size=23&letterSpacing=0&pause=1000&color=FFB478&center=true&width=435&lines=thank+you+for+reading+!!" alt="Typing SVG" /></a>
<p align="center"><img width="1640" height="250" alt="tumblr_f794596002bf19b63b81441a8c1925b8_56e8b460_2048" src="https://github.com/user-attachments/assets/77aa36c5-2126-40e3-901d-71b5a00c698a" />






 
  
  


 



